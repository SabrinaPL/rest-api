...
class GenderDataModel(BaseDocument):
    ...
    companies = m_engine.ListField(m_engine.StringField(), required=True) # List of production companies
    genres = m_engine.ListField(m_engine.StringField(), required=True) # List of genres (e.g., Action, Comedy, etc.)
    department = m_engine.StringField(required=True) # Department of the person (e.g., acting, writing, directing, etc.)
    gender = m_engine.IntField(default=None)  # 0 = unknown, 1 = female, 2 = male
