...
class MovieMetadata(BaseDocument):
    ...
    budget = m_engine.IntField(default=0)
    genres = m_engine.ListField(default=[])
    ...
    popularity = m_engine.FloatField(default=0.0)
    poster_path = m_engine.StringField(default="")  # 
    production_companies = m_engine.ListField(default=[])  
    production_countries = m_engine.ListField(default=[])  
    release_date = m_engine.DateTimeField()
    revenue = m_engine.IntField(default=0) 
    runtime = m_engine.IntField(default=0)
    spoken_languages = m_engine.ListField(default=[])
    ...
    vote_average = m_engine.FloatField(default=0.0)
    vote_count = m_engine.IntField(default=0)